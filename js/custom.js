...
function showHeadlineElements() {
	console.log($('#headline .info').children().addClass('loaded'));
}
...
function setupScrollFunctionality() {
	...
	function init(){
		...
		if ( $(document).scrollTop() >= swatchSection.top - $(window).height()-paddingForMobile){
			showSwatchSection();
		}
			
		if ( $(document).scrollTop() >= iconSection.top - $(window).height()+300-paddingForMobile){
			showIconSection();
		}
		
		if ( $(document).scrollTop() >= shareSection.top - $(window).height()+300-paddingForMobile){
			$("#counts").animateNumber(window.newNumber,{easing:"easeInOutCirc",duration:1000});
		}	
*/	
	}
	init();
...
var showResponsiveRunning = 0;
	function showResponsive(){
		if (showResponsiveRunning == 0){
			$('.responsive-iphone').css('left',0).css('opacity',1);
	...
	function showSwatchSection(){
		if (showSwatchSectionRunning == 0){
			$('#colorful-icon').css('background-color',"rgb(26, 188, 156)");
	...
	function showIconSection(){
		if (showIconSectionRunning == 0){
			...
			$('#icon-heart').animate({top:0, opacity:1,myRotationProperty:1},{
				duration:500,
